...
/*async function createBookings() {
  ...
  const { data: profilesIds } = await supabase
    .from("profiles")
    .select("id")
    .order("id");
  const allCabinIds = profilesIds.map((cabin) => cabin.id);

  const finalBookings = bookings.map((booking) => {
    // Here relying on the order of profiles, as they don't have and ID yet
    const cabin = profiles.at(booking.cabinId - 1);
